...
class TestRoundEvents:
    async def test_round_start_fires(
        ...
        calls: list[tuple[RoundState | None, RoundState]] = []
...
class TestPlayerEvents:
    async def test_kill_fires(
        ...
        calls: list[tuple[PlayerMatchStats | None, PlayerMatchStats]] = []
    ...
    async def test_death_fires(
        ...
        calls: list[tuple[PlayerState | None, PlayerState]] = []
...
class TestStateUpdateEvent:
    async def test_fires_on_every_valid_payload(
        ...
        calls: list[tuple[GameState | None, GameState]] = []
...
class TestMultipleEvents:
    async def test_round_end_bomb_explode_death_fires_all_three(
        ...
        round_end_calls: list[tuple[RoundState | None, RoundState]] = []
        bomb_exploded_calls: list[tuple[RoundState | None, RoundState]] = []
        death_calls: list[tuple[PlayerState | None, PlayerState]] = []
        state_update_calls: list[tuple[GameState | None, GameState]] = []

        @gsi.on_round_end
        async def on_round_end(old: RoundState | None, new: RoundState) -> None:
            round_end_calls.append((old, new))

        @gsi.on_bomb_exploded
        async def on_bomb_exploded(old: RoundState | None, new: RoundState) -> None:
            bomb_exploded_calls.append((old, new))

        @gsi.on_local_player_death
        async def on_death(old: PlayerState | None, new: PlayerState) -> None:
            death_calls.append((old, new))

        @gsi.on_state_update
        async def on_state(old: GameState | None, new: GameState) -> None:
            state_update_calls.append((old, new))
    ...
    async def test_multiple_handlers_for_same_event(
        ...
        @gsi.on_round_start
        async def handler_a(old: RoundState | None, new: RoundState) -> None:
            calls_a.append(new)

        @gsi.on_round_start
        async def handler_b(old: RoundState | None, new: RoundState) -> None:
            calls_b.append(new)
...
class TestHandlerErrorIsolation:
    async def test_failing_handler_does_not_block_others(
        ...
        @gsi.on_round_start
        async def bad_handler(old: RoundState | None, new: RoundState) -> None:
            raise RuntimeError("intentional test error")

        @gsi.on_round_start
        async def good_handler(old: RoundState | None, new: RoundState) -> None:
            calls.append(new)
    ...
    async def test_server_returns_200_on_handler_error(
        ...
        @gsi.on_state_update
        async def bad_handler(old: GameState | None, new: GameState) -> None:
            raise RuntimeError("intentional test error")
